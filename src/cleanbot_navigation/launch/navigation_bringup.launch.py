...
def generate_launch_description():
    ...
    # ====================== 启动参数（实机环境）======================
    # 实机环境默认关闭use_sim_time，autostart默认关闭
    ...
    declare_use_sim_time = DeclareLaunchArgument(
        'use_sim_time',
        default_value='false',  # 实机环境关闭仿真时间
    ...
    # 2. 启动SLAM Toolbox（异步模式，优化实机TF同步）
    ...
    amcl_node = Node(
        ...
        parameters=[
            amcl_config,
            {'use_sim_time': use_sim_time},
            {'transform_tolerance': 0.3},  # 实机环境较小的时间容差
            {'tf_buffer_duration': 10.0}  # TF缓存时长
        ],
        remappings=[
    ...
    controller_server = Node(
        ...
        remappings=[
            ...
            ('/odom', '/odometry/filtered')  # 订阅你的里程计话题
        ]
    )
    ...
    # 添加核心节点
    ld.add_action(rplidar_launch)  # 实机环境启动真实激光雷达
    ...
    # 打印提示
    ld.add_action(LogInfo(msg='========== CleanBot 实机导航启动成功 =========='))
